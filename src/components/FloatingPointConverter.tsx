...
const FloatingPointConverter = () => {
  ...
  const decimalToBinary32 = (decimal: number): FloatingPointInfo => {
    ...
    return { sign: sign === '0' ? '+' : '−', exponent, mantissa, normalized, decimal: decimal.toString() };
  };
  ...
  const binary32ToDecimal = (binary: string): FloatingPointInfo => {
    ...
    if (cleanBinary.length !== 32) throw new Error('IEEE 32-bit format requires exactly 32 bits');
  ...
  const binary64ToDecimal = (binary: string): FloatingPointInfo => {
    const cleanBinary = binary.replace(/\s/g, '');
    if (cleanBinary.length !== 64) throw new Error('IEEE 64-bit format requires exactly 64 bits');
    ...
    try {
      ...
      if (conversionType === 'decimal-to-binary') {
        if (!inputValue || isNaN(parseFloat(inputValue))) {
          setError(t.fpHintDecimal);
          setResult(null);
          return;
        }
        const decimal = parseFloat(inputValue);
        setResult(format === 'ieee32' ? decimalToBinary32(decimal) : decimalToBinary64(decimal));
      } else {
        if (!inputValue || !/^[01\s]+$/.test(inputValue)) {
          setError(format === 'ieee32' ? t.fpHint32 : t.fpHint64);
          setResult(null);
          return;
        }
        setResult(format === 'ieee32' ? binary32ToDecimal(inputValue) : binary64ToDecimal(inputValue));
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : t.arCalcError);
      setResult(null);
    }
  };

  const getFormatInfo = () => {
    if (format === 'ieee32') return { sign: 1, exponent: 8, mantissa: 23, total: 32 };
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-white mb-6">{t.fpTitle}</h2>

      <div className="bg-blue-900/30 border border-blue-700 rounded-lg p-4 mb-6">
        <h3 className="text-sm font-semibold text-blue-300 mb-2">IEEE 754 Standard</h3>
        <p className="text-sm text-blue-200">
          IEEE 754 {t.fpSignBit.toLowerCase().includes('sign') ? 'floating-point standard' : 'Fließkomma-Standard'}.
          {' '}{t.fpSignBit}: 1 {t.fpBit}, {t.fpExponent}: {info.exponent} {t.fpBits}, {t.fpMantissa}: {info.mantissa} {t.fpBits}.
        </p>
      </div>
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-semibold text-blue-300 mb-2">{t.fpFormat}</label>
            <select
              value={format}
              onChange={(e) => { setFormat(e.target.value as FloatingPointFormat); setResult(null); setError(''); }}
          ...
          <div>
            <label className="block text-sm font-semibold text-blue-300 mb-2">{t.fpDirection}</label>
            <select
              value={conversionType}
              onChange={(e) => { setConversionType(e.target.value as 'decimal-to-binary' | 'binary-to-decimal'); setResult(null); setError(''); }}
              className="w-full px-4 py-3 bg-slate-700 text-white rounded-lg border border-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              <option value="decimal-to-binary">{t.fpDecToBin}</option>
              <option value="binary-to-decimal">{t.fpBinToDec}</option>
            </select>
          </div>
        </div>
        ...
        <div>
          <label className="block text-sm font-semibold text-blue-300 mb-2">
            {conversionType === 'decimal-to-binary' ? t.fpLabelDecimal : t.fpLabelBinary}
          </label>
          <input
            type="text"
            value={inputValue}
            ...
            placeholder={
              conversionType === 'decimal-to-binary'
                ? 'e.g. 3.14 / z.B. 3.14'
                : format === 'ieee32'
                  ? 'e.g. 01000000010010001111010111000011'
                  : 'e.g. 0100000000001001000111101011100011010101110000101000101010101010'
            }
          ...
          <p className="text-xs text-slate-400 mt-1">
            {conversionType === 'decimal-to-binary'
              ? t.fpHintDecimal
              : format === 'ieee32' ? t.fpHint32 : t.fpHint64}
          </p>
        </div>
      ...
      {result && (
        <div className="border-t border-slate-700 pt-6 space-y-4">
          <h3 className="text-lg font-semibold text-white mb-4">{t.fpAnalysis}</h3>

          <div className="bg-slate-700 p-6 rounded-lg space-y-4">
            <div>
              <p className="text-sm text-blue-300 mb-2">{t.fpDecimalValue}</p>
            ...
            <div className="border-t border-slate-600 pt-4 space-y-3">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="inline-block bg-green-600 text-white text-xs font-bold px-2 py-1 rounded">
                    {t.fpExponent} ({info.exponent} {t.fpBits})
                  </span>
                  <p className="text-blue-300 text-sm">
                    {t.fpValue}: {parseInt(result.exponent, 2)} ({format === 'ieee32' ? t.fpBias127 : t.fpBias1023})
                  </p>
                </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="inline-block bg-blue-600 text-white text-xs font-bold px-2 py-1 rounded">
                    {t.fpMantissa} ({info.mantissa} {t.fpBits})
                  </span>
                  <p className="text-blue-300 text-sm">{t.fpMantissaLabel}</p>
                </div>
              ...
              <div className="border-t border-slate-600 pt-4">
                <p className="text-sm text-blue-300 mb-2">{t.fpNormalized}</p>
          ...
          <div className="bg-blue-900/30 border border-blue-700 rounded-lg p-4">
            <h4 className="text-sm font-semibold text-blue-300 mb-2">{t.fpFormatInfo}</h4>
            <div className="grid grid-cols-3 gap-3 text-sm">
              <div>
                <p className="text-blue-300">{t.fpSignBit.split('(')[0].trim()}</p>
                <p className="text-white font-bold">{info.sign} {t.fpBit}</p>
              </div>
              <div>
                <p className="text-blue-300">{t.fpExponent}</p>
                <p className="text-white font-bold">{info.exponent} {t.fpBits}</p>
              </div>
              <div>
                <p className="text-blue-300">{t.fpMantissa}</p>
                <p className="text-white font-bold">{info.mantissa} {t.fpBits}</p>
              </div>
            </div>
          </div>
        </div>
